calculInformacio <-function(matriu, Prob){
  B<-1
	
       number<-matrix(,4,ncol(matriu))
       R<-vector(mode='logical', length=ncol(matriu))
       for (ii in c(1:(ncol(matriu)))) {
               number[1,ii]<-sum(as.numeric(matriu[,ii]=="a" | matriu[,ii]=="A"));number[2,ii]<-sum(as.numeric(matriu[,ii]=="t" | matriu[,ii]=="T"))
               number[3,ii]<-sum(as.numeric(matriu[,ii]=="c" | matriu[, ii]=="C"));number[4,ii]<-sum(as.numeric(matriu[,ii]=="g" | matriu[, ii]=="G"))
