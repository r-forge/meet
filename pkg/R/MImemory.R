		...
		Divergence<-lapply(seq(1, length(interA), 1), function(k){
						...
						if (interA[k]==interB[k]){
						   diag(mi)<-sapply(c(1:length(nucleotids)),function(i){
											...
											pmX<-probability(training.set.mes.val, Prob, missing.fun)
											...
											D<-switch(iicc$classentropy, "Shannon"=divergence.Shannon(training.set.mes.val,H,HXY,correction),"Renyi"=divergence.Renyi(training.set.mes.val,pmX,pmXY,q,correction))
											zzmi<-D[1,1]
										})				
						}else{
