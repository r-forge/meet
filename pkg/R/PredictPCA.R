PredictPCA <-function(iicc){
  ...
  DetectedFactors<-cbind(which((1-Score)<=threshold),Score[(1-Score)<=threshold])
   ...
   if(nrow(DetectedFactors)==0){
      ...
      output<-lapply(c(1:nrow(DetectedFactors)),function(x){cbind(Sequence=paste(Sequence[DetectedFactors[x,1]:(DetectedFactors[x]+len_motif)],sep="",collapse=""),pvalue=DetectedFactors[x,2], position=DetectedFactors[x,1], direction=iicc$direction)})
    }
